...
const CollapsibleBibleReader: React.FC<CollapsibleBibleReaderProps> = ({ className = '', initialReference }) => {
  ...
  useEffect(() => {
    const loadVoices = () => {
      ...
      // Only set default voice on initial load (use a ref to track if we've set initial voice)
      setSelectedVoice(prevVoice => {
        // If we already have a selected voice, validate it still exists
        if (prevVoice) {
          const currentVoice = englishVoices.find(v => v.name === prevVoice.name)
          if (currentVoice) {
            return currentVoice // Voice still exists, keep it
          }
        }
        
        // No voice selected or voice no longer available, pick a default
        if (englishVoices.length > 0) {
          ...
          return femaleVoice || englishVoices[0] || null
        }
  ...
  // Handle initial reference from URL
  useEffect(() => {
    if (initialReference && !selectedBook) {
      const parsed = parseReference(initialReference)
      if (parsed) {
        // Set default version to KJV
        setSelectedVersion('de4e12af7f28f599-02') // KJV
        setSelectedBook(parsed.book)
        setSelectedChapter(parsed.chapter)
        if (parsed.verse) {
          setStartVerse(parsed.verse)
        }
        if (parsed.endVerse) {
  ...
  const parseContentForReading = (content: string): Array<{number: string, text: string, words: string[]}> => {
    ...
    return verses.map(verse => {
      ...
      // More aggressive pattern: match number at start (with or without space), then capture rest
      const verseMatch = trimmedVerse.match(/^(\d+)[\s]*(.+)$/s)
      
      if (verseMatch) {
        const [, verseNumber, verseText] = verseMatch
        // Aggressively remove verse number from text - handle cases like "1There" or "1 There"
        let cleanText = verseText
          .replace(/^\d+\s+/, '') // Remove "1 " pattern (with space)
          .replace(/^(\d+)([A-Z])/, '$2') // Remove "1T" pattern (number directly before capital letter)
          .replace(/^\d+/, '') // Remove any remaining leading number
          .trim()
        
        // Split words and filter out standalone numbers AND words that start with numbers
        const words = cleanText
          .split(/\s+/)
          .filter(word => {
            const trimmed = word.trim()
            return trimmed.length > 0 && 
                   !/^\d+$/.test(trimmed) && // Not a standalone number
                   !/^\d+[A-Z]/.test(trimmed) // Not starting with number+letter like "1There"
          })
          .map(word => {
            // If word starts with number+letter, remove the number part
            return word.replace(/^\d+([A-Z])/, '$1')
          })
        
        return { number: verseNumber, text: cleanText, words }
      } else {
        // Remove any leading numbers - be more aggressive
        let cleanText = trimmedVerse
          .replace(/^\d+\s+/, '') // Remove "1 " pattern
          .replace(/^(\d+)([A-Z])/, '$2') // Remove "1T" pattern
          .replace(/^\d+/, '') // Remove any remaining leading number
          .trim()
  ...
  const readVerse = (verseIndex: number, wordIndex: number = 0, readOnlyThisVerse: boolean = false) => {
    ...
    if ('speechSynthesis' in window) {
      ...
      // Set voice - ensure it's properly applied
      if (selectedVoice) {
        utterance.voice = selectedVoice
      } else {
        // Fallback: try to get a default voice if none selected
        const voices = speechSynthesis.getVoices()
        const englishVoices = voices.filter(voice => voice.lang.startsWith('en'))
        if (englishVoices.length > 0) {
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-full sm:w-32 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-flame-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white disabled:bg-gray-100 dark:disabled:bg-gray-800 disabled:cursor-not-allowed"
              />
              {(startVerse || endVerse) && (
                <button
                  onClick={() => {
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      // Re-fetch voices in case they've changed
                      const voices = speechSynthesis.getVoices()
                      ...
                      if (voice) {
                        ...
                        const currentVerse = currentVerseIndex
                        const wasSingleVerse = selectedVerseForReading !== null
                        ...
                        if (wasPlaying) {
                          stopReading()
                          // Small delay to ensure state is updated, then restart
                          setTimeout(() => {
                            if (currentVerse >= 0 && currentVerse < parsedVerses.length) {
                              startReading(currentVerse, wasSingleVerse)
                            } else {
                        ...
                        // Don't set to null, keep current selection
                      }
                    }}
                  ...
                  <div className="flex items-center gap-2">
                    {!isPlaying ? (
                      <button
                        ...
                        className="flex items-center gap-2 bg-flame-600 hover:bg-flame-700 text-white px-3 sm:px-4 py-2 rounded-lg transition-colors duration-200 text-sm sm:text-base"
                      >
                      ...
                      <div className="flex items-center gap-2">
                        {!isPaused ? (
                          <button
                            ...
                            className="flex items-center gap-2 bg-yellow-600 hover:bg-yellow-700 text-white px-3 sm:px-4 py-2 rounded-lg transition-colors duration-200 text-sm sm:text-base"
                          >
                          ...
                          <button
                            ...
                            className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-3 sm:px-4 py-2 rounded-lg transition-colors duration-200 text-sm sm:text-base"
                          >
                            <Play className="h-4 w-4" />
                        ...
                        <button
                          onClick={stopReading}
                          className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-3 sm:px-4 py-2 rounded-lg transition-colors duration-200 text-sm sm:text-base"
                        >
