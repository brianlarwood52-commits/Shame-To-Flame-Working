...
export default function AdminWith2FA() {
  ...
  const callEdgeFunction = async (functionName: string, options: {
    method?: string
    body?: any
    ...
    // Use session token if requested, otherwise use anon key
  ...
  useEffect(() => {
    ...
    if (savedToken) {
      ...
      // Don't auto-load data, let user refresh manually
    }
  }, [])
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await callEdgeFunction('admin-2fa-send', {
  ...
  const handleCodeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await callEdgeFunction('admin-2fa-verify', {
        body: { email: adminEmail, code: twoFactorCode },
      })
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      // Get session token from localStorage if not in state
      ...
      const data = await callEdgeFunction('admin-messages', {
        method: 'GET',
  ...
  const decryptMessage = async (item: ContactSubmission | PrayerRequest): Promise<string | null> => {
    ...
    try {
      ...
      const data = await callEdgeFunction('admin-decrypt', {
        body: {
